...
def _run_review_in_thread(
    ...
    try:
        ...
        with thread_scoped_silence():
            ...
            # Expose explicit external-memory writes without ingesting the
            # review prompt/output or running the parent's lifecycle hooks.
            parent_memory = getattr(agent, "_memory_manager", None)
            if parent_memory is not None:
                review_agent._memory_manager = _RetainOnlyMemoryProxy(parent_memory)
            ...
            review_memory = getattr(review_agent, "_memory_manager", None)
            if review_memory is not None:
                review_whitelist.update(
                    name for schema in review_memory.get_all_tool_schemas()
                    if (name := _RetainOnlyMemoryProxy._schema_name(schema))
                )
            ...
            set_thread_tool_whitelist(
                ...
                deny_msg_fmt=(
                    ...
                    "memory for notes, and external-memory retain tools when available." + _extra_deny_note
            ...
            try:
                ...
                if request_admitted:
                    ...
                    review_agent.run_conversation(
                        user_message=(
                            prompt
                            + "\n\nYou can only call memory, external-memory retain, "
                            "read_file/search_files, and skill management tools. Other tools will be denied "
