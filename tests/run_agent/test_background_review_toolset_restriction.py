...
def test_external_memory_retain_survives_review_without_automatic_ingestion():
    """The review can retain facts without storing its own harness conversation."""
    ...
    agent = _make_agent_stub(run_agent.AIAgent)
    memory = MagicMock()
    memory.get_all_tool_schemas.return_value = [
        {"type": "function", "function": {"name": name, "parameters": {"type": "object"}}}
        for name in ("hindsight_retain", "hindsight_recall")
    ]
    memory.has_tool.side_effect = lambda name: name in {"hindsight_retain", "hindsight_recall"}
    memory.handle_tool_call.return_value = "saved"
    agent._memory_manager = memory
    checked = []

    def review_turn(self, **kwargs):
        proxy = self._memory_manager
        assert proxy is not memory
        assert proxy.has_tool("hindsight_retain")
        assert not proxy.has_tool("hindsight_recall")
        assert get_pre_tool_call_block_message("hindsight_retain", {}) is None
        assert get_pre_tool_call_block_message("hindsight_recall", {}) is not None
        ...
        assert self._skip_mcp_refresh
        for hook in ("on_turn_start", "prefetch_all", "sync_all", "on_session_end", "shutdown_all"):
            getattr(proxy, hook)("review-only material")
            getattr(memory, hook).assert_not_called()
        assert proxy.handle_tool_call("hindsight_retain", {"content": "test fact"}) == "saved"
        assert "not available" in proxy.handle_tool_call("hindsight_recall", {})
        checked.append(True)
        return {"final_response": "Saved test fact."}

    with patch.object(run_agent.AIAgent, "__init__", lambda self, **kw: None), \
         patch.object(run_agent.AIAgent, "run_conversation", review_turn), \
         patch.object(run_agent.AIAgent, "shutdown_memory_provider", lambda self: None), \
         patch.object(run_agent.AIAgent, "close", lambda self: None), \
         patch("threading.Thread", _SyncThread):
        agent._spawn_background_review(messages_snapshot=[], review_memory=True, review_skills=False)

    assert checked == [True], "Review did not reach the retain-only dispatch assertions"
    memory.handle_tool_call.assert_called_once_with("hindsight_retain", {"content": "test fact"})
